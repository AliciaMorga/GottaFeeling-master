...
- (void) setValue:(NSObject *)newvalue {
	...
	if (valueindex == NSNotFound) {
		valuelabel.text = NSLocalizedStringFromTable(@"Config Error", stringsTable, nil);
	} else {
		valuelabel.text = NSLocalizedStringFromTable([[configuration objectForKey:@"Titles"] objectAtIndex:valueindex],
                                                     stringsTable,
                                                     nil);
	}
}
