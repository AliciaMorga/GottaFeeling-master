...
- (void) configurationFromPList:(NSString *)configfile {
    ...
    stringsTable = [[rootofplist valueForKey:@"StringsTable"] retain];  // PA: Added retain
	...
	for (NSDictionary *item in preferenceitems) {
		...
		if ([type isEqualToString:@"PSGroupSpecifier"]) {
			...
			[sectiontitles addObject:NSLocalizedStringFromTable([item valueForKey:@"Title"], stringsTable, nil)];
		} 
		// if not, add the item to current section 
			...
			/* set value to an empty string if it's not already defined */
				[settings setValue:([item valueForKey:@"DefaultValue"]
				                    ? [item valueForKey:@"DefaultValue"]
				                    : @"")
				            forKey:key];
		}
	...
	if ([rootofplist valueForKey:@"TitleKey"])
		title = NSLocalizedStringFromTable([settings valueForKey:[rootofplist valueForKey:@"TitleKey"]], stringsTable, nil);
	// @todo add observer to update title when settings.[rootofplist valueForKey:@"Titlekey"] changes.
	else
		title = NSLocalizedStringFromTable([rootofplist valueForKey:@"Title"], stringsTable, nil);	
}
...
- (void) dealloc {
	...
	[settings release];
    [stringsTable release];                             // PA: Added release
	[super dealloc];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
	...
	// Set up the cell...
	cell.stringsTable = stringsTable;
